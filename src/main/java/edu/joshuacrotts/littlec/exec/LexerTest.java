 ...
 * and Linux systems. This version should be portable, allowing the same test
 * cases to work, regardless of whether they are run on a Windows or a Linux
 * system.
 *
...
public class LexerTest {
  ...
  /**
   * Produces a "portable" string representation of the token and position. This
   * is necessary due to differences between Windows and Linux/Unix end of line
   * conventions, which makes the character positions of tokens different,
   * depending on what kind of system it is run on. The difference between this
   * output and the "grun" output is simply that this one does not print absolute
   * character token position offsets.
   *
  ...
  private static String portable(Lexer r, CommonToken tok) {
    ...
    return "[@" + tok.getTokenIndex() + "," + "'" + txt + "',<" + typeString + ">" + channelStr + "," + tok.getLine()
        + ":" + tok.getCharPositionInLine() + "]";
  }

  /**
   * Internal lexer tokenizer/print loop. Called from one of the public methods
   * after establishing a CharStream.
   *
